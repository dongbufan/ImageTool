<?php
namespace ImageTools\ImageUtil; 

class ImageUtil{
	public static $thumbPrefix = 'thumb_'; //缩略图前缀
	public static $waterPrefix = '5k3z4_'; //水印图片前缀
	private static $bi = 0.35; //缩略图所占高度比例
	private static $bi2 = 0.4; //缩略图所占高度比例
	 ...
	 * @return 
	 */
	public static function watermark($image,$water,$postion=9,$path='',$move_path=false){
		//获取源图和水印图片信息
		...
		$src_w = $dst_w*$bi;
		$src_h = ($src_w/$tmp_w)*$tmp_h;
		//获取各图片对应的创建函数名
		$dst_create_fname = self::$from[$dst_info['mime']];
		$src_create_fname = self::$from[$water_info['mime']];
		//使用可变函数来创建画布资源
		...
		imagecopyresampled($dst_img, $src_img, $dst_x, $dst_y, 0, 0, $src_w, $src_h,$tmp_w,$tmp_h);

		//生成带水印的图片
		$waterfile = $path.self::$waterPrefix.basename($image);
		$generate_fname = self::$to[$dst_info['mime']];
		
		if ($generate_fname($dst_img,$waterfile)){
			if($move_path){
				$move_path .= substr($path,-11,11);
				if(!is_dir($move_path)){
					mkdir($move_path,0777,true);
				}
				if(copy($image,$move_path.basename($image))){
					unlink($image);
				}else{
					die('{status:500,desc:"error"}');
				}
			}
			return $waterfile;
		} else {
	...
	public function thumbnail($image,$max_width,$max_height,$path=''){
		...
		//使用可变函数创建源图资源
		$src_create_fname = self::$from[$info['mime']];
		...
		$thumbfile = self::$thumbPrefix . pathinfo($image,PATHINFO_BASENAME);
		//获取存放上传文件的目录
			$sub_path = date('Ymd').'/';
		...
		$generate_fname = self::$to[$info['mime']];
		if ($generate_fname($dst_img,$path . $sub_path . $thumbfile)) {
